    ...
    return <div className="w-full bg-[var(--surface-1)]">
        <div className="grid w-full h-fit min-h-40 grid-cols-[60px_minmax(0,1fr)] grid-rows-[auto_auto] gap-x-3 gap-y-3 p-2">
            ...
            <header className="flex flex-col gap-2 w-full justify-center">
                <div className="flex items-center justify-center max-w-full w-fit gap-2 min-w-0 min-md:w-full">
                    <h1 className="text-md font-bold truncate min-w-0 flex-1 max-w-fit">
                        {curseforgeMod.name}
                    </h1>
                    <Separator orientation="vertical" />
                    <p className="text-md truncate min-w-0 flex-1 max-w-fit">
                        by {curseforgeMod.authors[0].name}
                    </p>
                </div>  
                <p className="text-sm text-left line-clamp-2 min-w-0 w-full">{curseforgeMod.summary}</p>
            </header>
            <div className="flex items-center justify-between w-full h-fit col-span-2">
                <div className="flex items-center justify-center flex-wrap gap-2">
                    <span className="flex items-center justify-center gap-1 text-center">
                        <Download /> {formattedDownloadCount}
                    </span>
                    <span className="flex items-center justify-center gap-1 text-center">
                        <RefreshCcw /> {timeSinceCurDate(curseforgeMod.dateModified)}
                    </span>
                    <span className="flex items-center justify-center gap-1 text-center">
